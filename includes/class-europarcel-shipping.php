...
class EuroParcelComWC_Shipping_Method extends WC_Shipping_Method {
    ...
    public function init_form_fields() {
        ...
        if (isset($post_data['woocommerce_' . $this->id . '_api_key']) && $this->instance_id) {
        ...
        if (!isset($this->settings['europarcel_customer']) || !$this->settings['europarcel_customer']) {
            ...
            if (get_option($settings_key)) {
                ...
                add_option($settings_key, $settings_value);
            }
        }
    ...
    public function calculate_shipping($package = array()) {
        ...
        if (!empty($customer->get_home_carriers())) {
            if ($has_free_shipping_to_home) {
                $this->add_rate(array(
                    'id' => $this->id . ':' . $this->instance_id . '_free_h2h',
                    'label' => $home_label,
                    'cost' => 0,
                    'package' => $package,
                    'meta_data' => [
